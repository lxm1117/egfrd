...
namespace peer {
    namespace wrappers {
        ...
        private:
            ...
            typedef stl_iterator_wrapper< typename policy_type::const_iterator, boost::python::handle<>, Trcg_> iterator_wrapper_type;
        ...
        public:
            ...
            {
                return iterator_wrapper_type::create(*self->impl_, boost::python::handle<>(boost::python::borrowed(const_cast<PyObject*>(reinterpret_cast<PyObject const*>(self)))));
            }

            static PyTypeObject* __class_init__(const char* name, PyObject* mod = 0)
            {
                using namespace boost::python;
                iterator_wrapper_type::__class_init__((std::string(name) + ".Iterator").c_str(), mod);
                if (__name__.empty())
                {
                    __name__ = mod && PyModule_Check(mod) ? extract<std::string>(object(borrowed(mod)).attr("__name__"))() + "." + name : std::string(name);
