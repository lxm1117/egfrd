...
namespace binding {
    ...
    inline void register_reaction_rule_class(const char* name)
    {
        ...
        peer::util::to_native_converter<typename impl_type::Reactants, seq_to_reactants_converter<impl_type> >();
        peer::converters::register_range_to_tuple_converter<typename impl_type::Reactants>();
        peer::converters::register_iterable_to_range_converter <species_type_id_vector>();

        // defining the class
        class_<impl_type>(name,
            init<typename impl_type::Reactants const&, species_type_id_vector>())
            .add_property("reactants", make_function(&impl_type::get_reactants, return_value_policy<copy_const_reference>()))
            .add_property("products", range<return_value_policy<return_by_value>, impl_type*>(&extras_type::get_products_begin, &extras_type::get_products_end))
