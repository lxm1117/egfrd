...
Real expxsq_erfc(Real x)
{
    ...
    if (x > 26.0)
    {
        ...
        result = (M_1_SQRTPI / x) * (1.0 - x2sq_r + x2sq_r * x2sq_r);
    }
    else
    {
...
Real XP10(Real r, Real t, Real r0, Real D, Real v)
{
    ...
    return drift_prefac / sqrt(fourDt * M_PI) * (exp(-rminr02 / fourDt) - exp(-rplusr02 / fourDt));
}
...
Real XP20(Real r, Real t, Real r0, Real D, Real v)
{
    ...
    const Real XP20_nov(1. / sqrt(fourDt * M_PI) * (exp(-rminr02 / fourDt) + exp(-rplusr02 / fourDt)));
    ...
    else
    {
        ...
        const Real XP20_v(exp(v_2 / D * (r + r0) + v_2 * v_2 / D * t) * erfc((r + r0 + v * t) / sqrt4Dt));
...
Real XP30term_v(Real r, Real t, Real r0, Real ka, Real D, Real v)
{
    ...
    const Real exp_arg(1.0 / D * (kplusv2 * kplusv2 * t + kplusv2 * (r + r0)));
...
Real XP30(Real r, Real t, Real r0, Real ka, Real D, Real v)
{
    ...
    else
    {
        ...
        return drift_prefac * (XP20temp + 1 / D * (ka + v / 2) * XP30term_v(r, t, r0, ka, D, v));
    }
}
...
Real XS30(Real t, Real r0, Real ka, Real D, Real v)
{
    ...
    else
    {
        ...
        const Real term2(erfc(-r0plusvt / sqrt4Dt) - ka / (ka + v) * exp(-v / D * r0) * erfc(r0minvt / sqrt4Dt));
...
Real XI030(Real r, Real t, Real r0, Real ka, Real D)
{
    ...
    return XI00(r, t, r0, D, 0.0) + .5 * ((XS30(t, r0, ka, D, 0.0) - 1) + sign * XI30term_nov(fabs(r), t, r0, ka, D));
}
...
Real __p_irr(Real r, Real t, Real r0, Real kf, Real D, Real sigma, Real alpha)
{
    ...
    const Real num3(W(r_plus_r0_minus_2sigma / std::sqrt(Dt4), alpha * std::sqrt(t)));
...
Real p_irr(Real r, Real t, Real r0, Real kf, Real D, Real sigma)
{
    ...
    return __p_irr(r, t, r0, kf, D, sigma, alpha);
}
...
Real __p_reaction_irr(Real t, Real r0, Real kf, Real D, Real sigma, Real alpha, Real kD)
{
    ...
    const Real sqrtD(std::sqrt(D));
    const Real r0_m_sigma_over_sqrt4D_t((r0 - sigma) / ((sqrtD + sqrtD) * sqrtt));
...
Real p_survival_nocollision(Real t, Real r0, Real D, Real a)
{
    ...
    const uint i_max(std::max(static_cast<uint>(std::ceil(std::sqrt(M_PI * M_PI + asq * std::log(1.0 / TOLERANCE) / Dt) *        M_1_PI)), 2u));
    ...
    while (true)
    {
        const Real term(sign * std::exp(exp_factor * i * i) * std::sin(angle_factor * i) / i);
...
Real ip_theta_free(Real theta, Real r, Real r0, Real t, Real D)
{
    ...
    const Real term1(expm1(rr0_over_2Dt - rsqr0sq_over_4Dt));
    const Real term2(expm1(rr0_over_2Dt * cos(theta) - rsqr0sq_over_4Dt));
...
Real g_bd_3D(Real r, Real sigma, Real t, Real D)
{
    ...
    const Real term1((std::exp(rps * rps * mDt4_r) - std::exp(rms * rms * mDt4_r)) * sqrtDt4 / (sqrtPi * r));
...
Real I_bd_3D(Real sigma, Real t, Real D)
{
    ...
    const Real result(term1 * (-sqrtDt * (term2 * std::exp(-sigmasq / Dt) + term3) + term4));
...
{
    return I_bd_r_3D(r, params->sigma, params->t, params->D) - params->target;
}

static Real I_gbd_r_1D_F(Real r, const g_bd_params* params)
{
    return I_bd_r_1D(r, params->sigma, params->t, params->D, params->v) - params->target;
}
...
Real drawR_gbd_3D(Real rnd, Real sigma, Real t, Real D)
{
    ...
    gsl_function F = { reinterpret_cast<double(*)(double, void *)>(&I_gbd_r_3D_F), &params };
...
Real drawR_gbd_1D(Real rnd, Real sigma, Real t, Real D, Real v)
{
    ...
    gsl_function F = { reinterpret_cast<double(*)(double, void*)>(&I_gbd_r_1D_F), &params };
