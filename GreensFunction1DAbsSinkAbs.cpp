...
uint GreensFunction1DAbsSinkAbs::guess_maxi(Real const& t) const
{
    ...
    const uint maxi(safety +
        static_cast<uint>(max_root * (getLr() + getLl()) / M_PI));
...
Real GreensFunction1DAbsSinkAbs::flux_tot_i(uint i, Real const& t) const
{
    ...
    return root_i2 * exp(-getD() * t * root_i2) * p_survival_table_i(root_i);
}
...
Real GreensFunction1DAbsSinkAbs::p_int_r_leftdomain(uint i, Real const& rr, Real const& t, RealVector& table) const
{
    ...
    const Real temp(getD() * sin(root_i * LrmL0) * (cos(root_i * Llprr) - 1.0));
...
Real GreensFunction1DAbsSinkAbs::drawR(Real rnd, Real t) const
{
    ...
    gsl_root_fsolver* solver(gsl_root_fsolver_alloc(solverType));
    Real r(findRoot(F, solver, getsigma(), geta(), EPSILON*L, EPSILON, "GreensFunction1AbsSinkAbs::drawR"));
    gsl_root_fsolver_free(solver);
