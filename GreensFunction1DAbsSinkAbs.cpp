...
const uint GreensFunction1DAbsSinkAbs::MAX_TERMS = 500;
const uint GreensFunction1DAbsSinkAbs::MIN_TERMS = 20;
const Real GreensFunction1DAbsSinkAbs::CUTOFF_H = 6.0;

Logger& GreensFunction1DAbsSinkAbs::log_(Logger::get_logger("GreensFunction1DAbsSinkAbs"));
...
{
    const Real root_n(std::max(lo_up_params.last_long_root, lo_up_params.last_short_root));
    ...
    if (f_upper * parity_op < 0)
    {
        ...
        if (cntr >= 10)
        {
            ...
            if (cntr >= 10)
            {
                log_.warn("Failed to straddle root# %6u. ", rootList_size() + 1);
                log_.warn("next root est. = %.16g with stepsize: %.16g, ll: %.16g, ls: %.16g.", next_root_est, delta, lo_up_params.last_long_root, lo_up_params.last_short_root);
                log_.warn("f_low( %.16g ) =  %.16g, f_high( %.16g ) = %.16g.", lower, f_lower, upper, f_upper);
            }
        }
    }

    return RealPair(lower, upper);
}
...
uint GreensFunction1DAbsSinkAbs::guess_maxi(Real const& t) const
{
    ...
    const uint maxi(safety + static_cast<uint>(max_root * (Lr + Ll) / M_PI));
    return std::min(maxi, MAX_TERMS);
}

/* Standard form of the greensfunction without numerator */
inline Real GreensFunction1DAbsSinkAbs::p_exp_den_i(Real const& t, Real const& root_i, Real const& root_i2) const
{
    return exp(-D * root_i2 * t) / p_denominator_i(root_i);
}
...
Real GreensFunction1DAbsSinkAbs::p_survival_i(uint i, Real const& t, RealVector const& table) const
{
    const Real root_i(get_root(i));
    return exp(-D * t * gsl_pow_2(root_i)) * table[i];
}
...
Real GreensFunction1DAbsSinkAbs::p_survival_table_i(Real const& root_i) const
{
    ...
    const Real term1(sin(root_i * L) - sin(root_i * LrmL0) - sin(root_i * (Ll + L0)));
    const Real term2(sin(root_i * Lr) - sin(root_i * L0) - sin(root_i * LrmL0));
...
Real GreensFunction1DAbsSinkAbs::prob_r_r0_i(uint i, Real const& rr, Real const& t) const
{
    const Real root_i(get_root(i));
    Real L0(getL0());       // copy!
    Real rr2(rr);
    ...
    Real numerator(D * root_i * sin(root_i * LlpL0) + k * sin(root_i * Ll) * sin(root_i * L0));
...
Real GreensFunction1DAbsSinkAbs::prob_r_nor0_i(uint i, Real const& rr, Real const&t) const
{
    const Real root_i(get_root(i));
    const Real LrmL0(Lr - L0);
    const Real Llprr(Ll + rr);
    const Real numerator(D * root_i * sin(root_i * Llprr) * sin(root_i * LrmL0));
    return -2.0 * p_exp_den_i(t, root_i, gsl_pow_2(root_i)) * numerator;
}
...
Real GreensFunction1DAbsSinkAbs::prob_r(Real r, Real t) const
{
    ...
    if (t == 0 || D == 0) return r == r0 ? INFINITY : 0.0;            // the probability density function is a delta function

    // if r is at one of the the absorbing boundaries
    if (fabs(a - r) < EPSILON * L || fabs(r - sigma) < EPSILON * L) return 0.0;

    const Real rr(r0 - rsink >= 0 ? r - rsink : rsink - r);
...
Real GreensFunction1DAbsSinkAbs::flux_tot_i(uint i, Real const& t) const
{
    ...
    return root_i2 * exp(-D * t * root_i2) * p_survival_table_i(root_i);
}
...
Real GreensFunction1DAbsSinkAbs::flux_abs_Lr_i(uint i, Real const& t) const
{
    ...
    Real numerator(k * sin(root_i * Ll) * sin(root_i * L0) + D * root_i * sin(root_i * LlpL0));
    numerator *= root_i;
...
GreensFunction::EventKind GreensFunction1DAbsSinkAbs::drawEventType(Real rnd, Real t) const
{
    ...
    if (maxi == MAX_TERMS) log_.info("drawEventType: maxi was cut to MAX_TERMS for t = %.16g", t);
...
Real GreensFunction1DAbsSinkAbs::drawTime(Real rnd) const
{
    ...
    gsl_root_fsolver* solver(gsl_root_fsolver_alloc(solverType));
    const Real t(findRoot(F, solver, low, high, t_scale*EPSILON, EPSILON, "GreensFunction1DAbsSinkAbs::drawTime"));
    gsl_root_fsolver_free(solver);
    return t;
}
...
Real GreensFunction1DAbsSinkAbs::p_int_r_table(Real const& r, Real const& t, RealVector& table) const
{
    Real p;
    ...
    else
    {
        ...
        {
            return XI030(rr, t, L0, k, D);
        }
    }

    const uint maxi(guess_maxi(t));

    if (maxi == MAX_TERMS) log_.warn("p_int_r_table: maxi was cut to MAX_TERMS for t = %.16g", t);
...
Real GreensFunction1DAbsSinkAbs::p_int_r(Real const& r, Real const& t) const
{
...
void GreensFunction1DAbsSinkAbs::create_p_int_r_Table(Real const& t, RealVector& table) const
{
...
Real GreensFunction1DAbsSinkAbs::p_int_r_leftdomain(uint i, Real const& rr, Real const& t, RealVector& table) const
{
    const Real root_i(get_root(i));
    const Real LrmL0(Lr - L0);
    const Real Llprr(Ll + rr);
    const Real temp(D * sin(root_i * LrmL0) * (cos(root_i * Llprr) - 1.0));
    return get_p_int_r_Table_i(i, t, table) * temp;
}

//Integrated Greens function for rr part of (0, L0]
Real GreensFunction1DAbsSinkAbs::p_int_r_rightdomainA(uint i, Real const& rr, Real const& t, RealVector& table) const
{
    const Real root_i(get_root(i));
    const Real LrmL0(Lr - L0);
    const Real Llprr(Ll + rr);
    const Real root_i_rr(root_i * rr);
    const Real temp(D * (cos(root_i * Llprr) - 1.0) + k / root_i * (cos(root_i_rr) - 1.0) * sin(root_i * Ll));
    return get_p_int_r_Table_i(i, t, table) * sin(root_i * LrmL0) * temp;
}

//Integrated Greens function for rr part of (L0, Lr]
Real GreensFunction1DAbsSinkAbs::p_int_r_rightdomainB(uint i, Real const& rr, Real const& t, RealVector& table) const
{
    const Real root_i(get_root(i));
    const Real L(Lr + Ll);
    const Real LrmL0(Lr - L0);
    ...
    const Real term1(sin(root_i * L) - sin(root_i * LrmL0) - sin(root_i * LlpL0) * cos(root_i * Lrmrr));
    const Real term2(sin(root_i * Lr) - sin(root_i * LrmL0) - sin(root_i * L0) * cos(root_i * Lrmrr));
    const Real temp(D * term1 + k * sin(root_i * Ll) * term2 / root_i);
...
Real GreensFunction1DAbsSinkAbs::drawR(Real rnd, Real t) const
{
    ...
    if (t == 0.0 || D == 0.0) return r0;// the trivial case
    if (L < 0.0) return 0.0;// if the domain had zero size
    if (rnd <= EPSILON) return sigma;
...
std::string GreensFunction1DAbsSinkAbs::dump() const
{
    ...
    ss << "D = " << D << ", sigma = " << sigma << ", a = " << a << ", r0 = " << r0 << ", rsink = " << rsink << ", k = " << k << std::endl;
    return ss.str();
}
