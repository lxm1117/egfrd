...
Logger& GreensFunction3D::log_(Logger::get_logger("GreensFunction3D"));
...
Real GreensFunction3D::drawR(Real rnd, Real t) const
{
    ...
    const Real r_range(H * sqrt(6.0 * D * t));
    ...
    for (uint i(0);;++i)
    {
        ...
        if (i >= maxIter) throw std::runtime_error("GreensFunction3D: drawR: failed to converge");
    }
...
Real GreensFunction3D::drawTheta(Real rnd, Real r, Real t) const
{
    ...
    for (uint i(0);;++i)
    {
        ...
        if (status != GSL_CONTINUE) break;
        if (i >= maxIter) throw std::runtime_error("GreensFunction3D: drawTheta: failed to converge");
    }
