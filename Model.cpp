...
GFRD_CLASS Model::Model() : network_rules_(new BasicNetworkRulesImpl())
{
    
}


GFRD_CLASS void Model::add_species_type(boost::shared_ptr<species_type_type> const& species)
{
...
GFRD_CLASS  boost::shared_ptr<Model::species_type_type> Model::get_species_type_by_id(species_id_type const& id) const
...
GFRD_CLASS Model::species_type_range Model::get_species_types() const
...
GFRD_CLASS std::string const& Model::operator[](std::string const& name) const
// Gets the name of the species? Or gets the value of an attribute by name?
{
    string_map_type::const_iterator i(attrs_.find(name));
    if (i == attrs_.end()) throw not_found((boost::format("key %s not found") % name).str());
    return (*i).second;
}
