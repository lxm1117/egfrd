...
struct ParticleContainerUtils
{
    ...
    struct distance_comparator : public std::binary_function <typename Tobject_and_distance_list_::placeholder,typename Tobject_and_distance_list_::placeholder, bool >
    {
...
class ParticleContainerBase : public ParticleContainer < Ttraits_ >
{
...
public:
    ParticleContainerBase(length_type world_size, size_type size) : pmat_(world_size, size), structures_() {}
    //ParticleContainerBase(length_type world_size, size_type size, structure_id_type default_structid) : pmat_(world_size, size), structures_(default_structid) {}
    ...
    T1_ calculate_pair_CoM(T1_ const& p1, T1_ const& p2, typename element_type_of<T1_>::type const& D1, typename element_type_of<T1_>::type const& D2)
    {
    ...
    particle_id_pair_and_distance_list* check_overlap(Tsph_ const& s, Tset_ const& ignore, typename boost::disable_if<boost::is_same<Tsph_, particle_id_pair> >::type* = 0) const
    {
    ...
    particle_id_pair_and_distance_list* check_overlap(Tsph_ const& s, typename boost::disable_if<boost::is_same<Tsph_, particle_id_pair> >::type* = 0) const
    {
    ...
    void surface_overlap_checker(particle_shape_type const& s, position_type const& old_pos, structure_id_type const& current, length_type const& sigma, Tfun_& checker) const
    {
        ...
        for (typename structure_id_set::const_iterator i(visible_structure_IDs.begin()), e(visible_structure_IDs.end()); i != e; ++i)
        {
        ...
        for (typename structure_map::const_iterator i(visible_structures.begin()), e(visible_structures.end()); i != e; ++i)
        {
...
inline Transaction<Ttraits_>* ParticleContainerBase<Tderived_, Ttraits_>::create_transaction()
{
